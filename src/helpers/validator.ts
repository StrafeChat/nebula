...
export const verifyToken = async (req: Request, res: Response & { locals: { user: User } }, next: NextFunction) => {
    ...
    if (users.rows[0].get("last_pass_reset") != parseInt(last_pass_reset) || users.rows[0].get("secret") != secret) return res.status(401).json({ message: "Unauthorized" });
    res.locals.user = users.rows[0] as unknown as User;
    next();
}